          ...
          <div className={style['label-menu-group']}>
            {changeLabel ? (
              <div
                className={
                  style[
                    `label-${changeLabel.toLocaleLowerCase().replace(' ', '-')}`
                  ]
                }
                onClick={() => handleLabelClick(issue_number, 'label')}
              >
                {changeLabel}
              </div>
            ) : (
              <div
                className={
                  style[`label-${label.toLocaleLowerCase().replace(' ', '-')}`]
                }
                onClick={() => handleLabelClick(issue_number, 'label')}
              >
                {label}
            ...
            {modalState?.menuState.id === issue_number &&
              modalState?.menuState.menu === 'label' && (
                <Menu
                  menuName="label"
                  menuList={labelList}
                  onMenuItemClick={handleLabelMenuItemClick}
                />
              )}
          </div>
          <div className={style['dot-menu-group']}>
            <div
              className={style['dot-icon']}
              onClick={() => handleDotClick(issue_number, 'dot')}
            >
              <BiDotsVertical />
            </div>
            {modalState?.menuState.id === issue_number &&
              modalState?.menuState.menu === 'dot' && (
                <Menu
                  ...
                  onMenuItemClick={handleDotMenuItemClick}
                />
              )}
          </div>
        </div>
